...
<span><strong>CONSULTA EQP ENVIADO Á BASE</strong></span><br><br><br>
...
$sql = mysql_query ("select status.protocolo,patrimonio,patrimonio.tecnico,patrimonio.ga,patrimonio.base,patrimonio.status_equip,patrimonio.equip,patrimonio.modelo from patrimonio join status on patrimonio.patrimonio = status.id_patrimonio where status_equip = 9   and data_recebimento_ga = '0000-00-00' and patrimonio.id_ga = '".$_SESSION['id']."' " );
...
$sql3 =  mysql_query ("select  count(*) as conta from patrimonio join status on patrimonio.patrimonio = status.id_patrimonio where status_equip = 9   and data_recebimento_ga = '0000-00-00' and patrimonio.id_ga = '".$_SESSION['id']."'" );
...
<div class="modal fade" id="myModal<?php echo $dado ['protocolo'];  ?>" role="dialog">
  ...
  <form role="form" name="seachform" method="post" action="conf_rec_base.php " >
...
<!--   <a href="http://zxing.appspot.com/scan?ret=http://192.168.43.31/modem/tecnico.php?codigo={CODE}">LEITOR </a> -->
 ...
 <input type="hidden"  class="form-control" name="m" value=" <?php echo $patri ?>"  id="m" required  >
  ...
  <button type="submit" value="Baixar" class="btn btn-default"> <strong>RECEBER</strong> </button>
